...
```
#### 1.3 Add additional variables
```{r}
# add race variable: the coding on race is a bit messy in Canpath
# 14 variables were used to indicate diff racial minorities
# The cleaning code is provided by M.Knight
m2<-merge(m2, select(Canpath_qx, "ResearcherID",
         ...
         by.x = 'rid', by.y ="ResearcherID", 
         all.x = T, all.y = F
  
)

# Reorder the variables to fit Matthew's code:
m3<-cbind.data.frame(m2[,1:8], m2[,17:30], m2[,9:16])
m2<-m3
rm(m3)

# If possible, add a short para in the discussion section explaining why we can not categorize the indigenous people. Consult Yuan's email for more detail.
```

```{r}
# define whites and minorities:

#Racialized minorities - no indigenous responses included (applied later in for loop)
...
```{r}
# a for loop for creating race var:
m2$race<-NULL
...
for(i in 1:nrow(m2)){
  m2$race[i]<-case_when(
    ...
    !is.na(m2[i,22]) & 
      sum(is.na(m2[i,9:22])) == 13 ~ "pnts",
    
    #select only 1 white ethnicity
    !is.na(m2[i,19]) & 
      sum(is.na(m2[i,9:22])) == 13 ~ "White",
    
    #select only 1 racialized minority ethnicity
    sum(!is.na(m2[i,9:18])) == 1 &
      sum(is.na(m2[i,9:22])) == 13 ~ "Racialized minority",
    
    #select 1 white ethnicity and at least 1 racialized minority ethnicity
    sum(!is.na(m2[i,9:18])) > 0 & 
      !is.na(m2[i,19]) &
      sum(is.na(m2[i,22])) ~ "Racialized minority",
    
    #select 2+ racialized minority ethnicities
    sum(!is.na(m2[i, 9:18])) > 1 &
      sum(is.na(m2[i, c(19, 22)])) == 2 ~ "Racialized minority",
    
    #select at least 1 racialized minority ethnicity and at least 1 "other" ethnicity
    sum(!is.na(m2[i, 9:18])) > 0 &
      sum(is.na(m2[i, c(19, 22)])) == 2 &
      sum(!is.na(m2[i, 20,21])) > 0 ~ "Racialized minority",
    
    #select white, but also identify as a racialized minority using text box
    sum(is.na(m2[i, c(9:18, 22)])) == 11 &
      !is.na(m2[i,19]) &
      sum(!is.na(m2[i, c(20:21)])) >= 1 &
      m2[i, c(21)] %in% (rm_indn) ~ "Racialized minority",
    
    #select white, but also identify as white using text box
    sum(is.na(m2[i, c(9:18, 22)])) == 11 &
      !is.na(m2[i,19]) &
      sum(!is.na(m2[i, c(20:21)])) >= 1 &
      m2[i, c(21)] %in% (w) ~ "White",
    
    #select other and identify as a racialized minority using text box
    sum(is.na(m2[i, c(9:19, 22)])) == 12 &
      m2[i, 21] %in% (rm_indn) ~ "Racialized minority",
...
table(m2$race, useNA = 'ifany')

# pnt = prefer not to answer
table(m2[,22], m2$race, useNA = 'ifany')

# remove loose race vars
m2<-select(m2, -c(9:22))
```
...
# Save to the shared folder as .csv format
write.csv(Canpaths, file = "~/../../jmy/CANPATH/Canpaths.csv", row.names=FALSE)
```
