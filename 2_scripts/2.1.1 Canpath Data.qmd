...
ser_rid_dup$id<-c(1:404)
ser_rid_dup %>%
  distinct(rid, .keep_all=TRUE) # N=163 （404-163 = 241 de-duplicated)
...
table(m2$sid_match, useNA = 'ifany')
...
##===================== Part.II Estimate Seroprevalence =================
...
M.Knight's data cleaning strategy is much more straightforward, as he only need to locate the participants who provided sero data, so matching on researcherID makes senses. In our case we need to treat sero data more carefully, as in Canpath sero data, one individual could provide multiple samples, and those samples were tested by multiple assays (Abbott N/S, Mount Sinai Hosp N/S/S-RBD). 
