...
```{r}
# Create month var
RFD4682_e$Col_Date<-RFD4682_e$COLLECTION_DATE %>%
  as.Date(., tz="UTC") # a short time var, date only

library(lubridate)
library(zoo)

# Only keep the month, remove the data
RFD4682_e$mois <- month(RFD4682_e$Col_Date)
RFD4682_e$annee <- year(RFD4682_e$Col_Date)
RFD4682_e$Col_Month<-paste(RFD4682_e$annee, RFD4682_e$mois, 
                           sep = "-")
...
# Sen=0.881, Spec = 0.947
# Source:
tb1$prop.n_RG<- (tb1$prop.n+0.881-1)/(0.881+0.947-1)
...
```{r}
tb1_melt<-melt(select(tb1, Col_Month, prop.n, prop.n_RG), id.vars = "Col_Month")
...
plot(x=temp$Time ,y=temp$Positivity, type = 'b', 
     col="blue", xlab="Time: 2022", ylab="Test Positivity",
     main="Anti-N Assay Positivity in Omicron Wave, using APL Data")
plot(x=temp$Time ,y=temp$Positivity_RG, 
     type = 'b', col="red")
```
