...
APLs<-RFD4682_e %>%
  select(order_ID, AGE_AT_COLLECTION, GENDER, N, Col_Month, PAT3)

# unify colnames
colnames(APLs)<-c("order_ID","age","gender","abbott_n","samplemonth","fsa")
...
#### 3.2 Apply the model lm01 to the CBS data
```{r}
...
#### 3.3 plot: sero-prevalence, using predicted Abbott positivity
```{r}
tb_abb<-CBSs %>%
  group_by(samplemonth, region) %>%
  summarise(n = n() , 
...
p04<-dta_all %>%
  ...
  ggplot(aes(x=log_roche_n, y=log_abbott_n, 
             study, colour=study)) +
  geom_point(aes(colour=as.factor(study))) +
  ...
  scale_colour_manual(labels = c("Jamaica (-)", "Osaka, JP (-)",
                                 "Osaka, JP (+)","UK (+)"), 
                      values = c("red", "skyblue", "darkgreen", "purple")) +
  theme_bw() + 
