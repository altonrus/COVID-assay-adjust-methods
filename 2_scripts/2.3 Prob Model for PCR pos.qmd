...
# How many obs we have in each provinces?
table(CBSs$province, useNA = 'ifany') # n(AB)=124008

# The APL data
load(file = "../1_data/private/RFD4682e.RData")   # ver.1
load(file = "../1_data/private/RFD4682ev2.RData") # ver.2 with additional anti-N data
```
...
APLs<-RFD4682_ev2 %>%
  select(order_ID, SERO_ID, clean_IDe,
         AGE_AT_COLLECTION, GENDER, `N-IgG_RESULT`,`N-IgG_INTERP`, COLLECTION_DATE)

# unify colnames
colnames(APLs)<-c("order_ID", "SERO_ID", "clean_IDe",
                  "age","gender","abbott_n", "pos_abbott","sampledate")

APLs$samplemonth<-as.yearmon(APLs$sampledate)

# add log_abbott_n
## find 0 values bf transformation, correct them as 0.001
APLs$abbott_n[APLs$abbott_n==0 & 
              !is.na(APLs$abbott_n)]<-0.001

APLs$log_abbott_n <- log(APLs$abbott_n)

# Recode abbott_n test results pos/neg as 1/0
...
Alton's comments: - solid/dash to indicate raw/transformed - red/blue to indicate Abbott/Roche - Merge the color & linetype indicator in the legend
...
p03.lab<-c("APL Abbott (actual)", "CBS Roche (actual)", 
           "APL Roche (transformed)", "CBS Abbott (transformed)")
p03.3<-tb_alberta %>%
...
```{r}
# rebuild the figure
p03.5lab<-c("APL Abbott (actual)", "CBS Roche (actual)", 
            "APL Abbott (RG-Bayesian)", "CBS Roche (RG-Bayesian)")
...
#### 3.6 Create fig panel for the manuscript
```{r}
# use cowplot package
p08.upper<-cowplot::plot_grid(p03.5  + ylab("Seroprevalence") +
                              ylim(0,0.8), 
                              p03.3  + ylab("") +
                              ylim(0,0.8)+
...
png(file="../4_output/figs/Fig.1m Seroprevalence.png", 
...
#### 4.3 RG Adjusted Sero-prevalence using Bayesian model's priors

##### 4.3.1 Create monthly anti-N positivity data for CBS
```{r}
# CBSs AB subset, N=124008
CBSs_AB<-CBSs[CBSs$province == "AB",]

# Monthly Positivity in CBS
tb_431<-CBSs_AB %>%
  group_by(samplemonth) %>%
  summarise(n = n() , 
            pos_roche= sum(pos_roche, na.rm = T)
            )
tb_431$prop.raw<-tb_431$pos_roche/tb_431$n

# Add RG adjustment
tb_431$prop.adj.p1<-(tb_431$prop.raw + 0.998 -1)/(0.998 + 0.979 -1)
tb_431$prop.adj.p2<-(tb_431$prop.raw + 0.993 -1)/(0.993 + 0.862 -1)
tb_431$study <- rep("CBS", length(tb_431$samplemonth))
```

##### 4.3.2 Create monthly anti-N positivity data for APL
```{r}
# APL data, N=214776

# Monthly Positivity in APL
tb_432<-APLs %>%
  group_by(samplemonth) %>%
  summarise(n = n() , 
            pos_abbott= sum(pos_abbott, na.rm = T)
            )
tb_432$prop.raw<-tb_432$pos_abbott/tb_432$n

# remove missing data
tb_432<-tb_432[!is.na(tb_432$samplemonth),]
tb_432<-tb_432[tb_432$pos_abbott!=0,]

# Add RG adjustment
tb_432$prop.adj.p1<-(tb_432$prop.raw + 0.995 -1)/(0.995 + 0.960 -1)
tb_432$prop.adj.p2<-(tb_432$prop.raw + 0.995 -1)/(0.995 + 0.838 -1)
tb_432$study <- rep("APL", length(tb_432$samplemonth))

# merge CBS and APL table
tb_43<-bind_rows(tb_431[, c(1, 4:7)], tb_432[-c(1), c(1, 4:7)])
# April 2020 row is removed

tb_43m<-reshape2::melt(tb_43, 
...
```{r}
p4.3.3<-tb_43m %>%
  ggplot(aes(x=samplemonth, y=prop, colour=study)) +
  geom_point(aes(colour=study)) +
  geom_line(aes(colour=study, linetype=adjustment))  +
  xlab("Month of Sample Collection") +
  ylab("Seroprevalence") +
  coord_cartesian(xlim = c(as.yearmon("May 2020"), as.yearmon("Jan 2022")),
                  ylim = c(0, 0.15)) +
#  ylim(c(as.yearmon("Jan 2022"), as.yearmon("Feb 2023"))) +
  scale_colour_manual("", values = c("red","blue")) +
  scale_linetype_manual("", values=c(1,2,3),
                        labels= c("Unadjusted", "RG adjusted (informed priors)",
                                  "RG adjusted (nautral priors)")) +
  theme_bw() 
