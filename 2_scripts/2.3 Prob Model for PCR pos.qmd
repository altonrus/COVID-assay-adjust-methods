...
# 12 provinces, recatgorized into five regions
CBSs$region<-rep(NA, length(CBSs$pid))
CBSs$region[CBSs$province %in% c("NL", "PE", "NS", "NB")]<-"Atlantic"
CBSs$region[CBSs$province %in% c("QC", "ON")]<-"Central"
CBSs$region[CBSs$province %in% c("AB", "MB", "SK")]<-"Prairie"
CBSs$region[CBSs$province %in% c("BC")]<-"BC"
CBSs$region[CBSs$province %in% c("NU/NT", "YT")]<-"North"
...
tb_ce<-CBSs[CBSs$region=="Central",] %>%
  group_by(samplemonth) %>%
  summarise(n = n() , ever_inf= mean(ever_inf, na.rm = T), 
            less_3_month=mean(less_3_month, na.rm = T))
tb_ce$region<-rep("Central", length(tb_ce$n))

tb_pr<-CBSs[CBSs$region=="Prairie",] %>%
  group_by(samplemonth) %>%
  summarise(n = n() , ever_inf= mean(ever_inf, na.rm = T), 
            less_3_month=mean(less_3_month, na.rm = T))
tb_pr$region<-rep("Prairie", length(tb_pr$n))

tb_no<-CBSs[CBSs$region=="North",] %>%
  group_by(samplemonth) %>%
  summarise(n = n() , ever_inf= mean(ever_inf, na.rm = T), 
            less_3_month=mean(less_3_month, na.rm = T))
tb_no$region<-rep("North", length(tb_no$n))

tb_all<-rbind.data.frame(tb_bc, tb_pr, tb_ce, tb_at, tb_no)
```
...
#  geom_line(aes(colour=inf))  +
  geom_smooth(method = "loess", aes(colour=inf)) +
  theme_bw() 
