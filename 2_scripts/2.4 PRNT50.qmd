...
tb_abbs_melt<-reshape2::melt(select(tb_abbs, 
                                    samplemonth, prop.abbs1, prop.abbs2), 
...
p09c<-tb_APLCBS2 %>%
  ...
  geom_point() +
  geom_line(aes(linetype = interaction(study, COI_ty)))  +
  theme_bw() +
...
dev.off()
```
##### Print p12 and p13 in pdf formats, as requested by AJE
```{r}
ggsave(file="../4_output/figs/Fig.S2 Vax.pdf", 
       width=2400, height=1200, dpi=300, units = "px", 
       plot = p1213)
```
...
ggsave(file="../4_output/figs/Fig.3 Immunity.pdf",
       plot = p09all,
        width=2550, height=2000, 
        dpi=300, units ="px")
```
