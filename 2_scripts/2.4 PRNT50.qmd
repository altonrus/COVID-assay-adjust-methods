...
tb_abbsv<-APLs %>%
  .[.$abb_s_AU==1 & !(.$vax %in% c("Not Immunized", "Pre-Immunization")) &
    .$DAYS_SINCE_IMM>=28 & !is.na(.$DAYS_SINCE_IMM),] %>%
  group_by(samplemonth) %>%
  summarise(n = n() , 
            pos_abbott_s= sum(pos_abbott_s, na.rm = T), 
            pos_abbott_s2= sum(pos_abbott_s2, na.rm = T)
            )
tb_abbsv$prop.abbs1<-tb_abbsv$pos_abbott_s/tb_abbsv$n
tb_abbsv$prop.abbs2<-tb_abbsv$pos_abbott_s2/tb_abbsv$n

# clean NAs
tb_abbsv<-tb_abbsv[!is.na(tb_abbsv$samplemonth),]

tb_abbsv_melt<-reshape2::melt(select(tb_abbsv, -c(2:4)), 
                        id.vars = c("samplemonth"),
                        variable.name = "COI",
                        value.name = "prop")
```
...
```{r}
tb_vax_est<-APLs %>%
  .[.$abb_s_AU==1 & !is.na(.$pos_abbott_s) & !is.na(.$pos_abbott),] %>%
  group_by(samplemonth) %>%
  summarise(n = n() , 
