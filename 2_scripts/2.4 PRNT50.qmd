...
tb_abbsv0<-APLs %>%
  .[.$abb_s_AU==1 & (.$vax %in% c("Not Immunized", "Pre-Immunization", "1 Dose")),] %>%
  group_by(samplemonth) %>%
  summarise(n = n() , 
            pos_abbott_s= sum(pos_abbott_s, na.rm = T), 
            pos_abbott_s2= sum(pos_abbott_s2, na.rm = T)
...
tb_abbsv1<-APLs %>%
  .[.$abb_s_AU==1 & .$vax %in% c("1-2 Doses","2 Doses","3 Doses",
                             "4 Doses") ,] %>%
  group_by(samplemonth) %>%
  summarise(n = n() , 
            pos_abbott_s= sum(pos_abbott_s, na.rm = T), 
...
```{r}
tb_abbsv11<-APLs %>%
  .[.$abb_s_AU==1 & .$vax %in% c("1 Dose") ,] %>%
  group_by(samplemonth) %>%
  summarise(n = n() , 
            pos_abbott_s= sum(pos_abbott_s, na.rm = T), 
            pos_abbott_s2= sum(pos_abbott_s2, na.rm = T)
            )
tb_abbsv11$prop.abbs1<-tb_abbsv11$pos_abbott_s/tb_abbsv11$n
tb_abbsv11$prop.abbs2<-tb_abbsv11$pos_abbott_s2/tb_abbsv11$n

# clean NAs
tb_abbsv11<-tb_abbsv11[!is.na(tb_abbsv11$samplemonth),]

tb_abbsv11_melt<-reshape2::melt(select(tb_abbsv11, -c(2:4)), 
                        id.vars = c("samplemonth"),
                        variable.name = "COI",
                        value.name = "prop")
```
